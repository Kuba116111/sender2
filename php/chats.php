<?php
    ...
    $sql = "SELECT * FROM chats WHERE friend1_id = '{$id}' OR friend2_id = '{$id}' ORDER BY date DESC";
    ...
    if(mysqli_num_rows($query) === 0){
        ...
        while ($row = mysqli_fetch_assoc($query)) {
            ...
            // echo $chats;
            // echo $one_chat;
            $sql_search = "SELECT * FROM messages_$chat_id";
            $query_search = mysqli_query($conn, $sql_search);
            
            if (mysqli_num_rows($query_search) > 0) {
                $sql2 = "SELECT * FROM messages_$chat_id ORDER BY id DESC";
                $sql3 = "SELECT * FROM messages_$chat_id ORDER BY id DESC";
